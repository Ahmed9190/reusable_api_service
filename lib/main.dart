...
import 'package:reusable_api_service/core/injection_container.dart';
...
final userDataSource =
    sl<ResourceApiService<UserModel, CreateUpdateUserModel>>();
final todoDataSource =
    sl<ResourceApiService<TodoModel, CreateUpdateTodoModel>>();
...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> tryUserApi() async {
    final users = await userDataSource.getAll();
    print("All users: $users");
    final firstUser = await userDataSource.getById(users.first.id);
    print("First user: $firstUser");
    final createdUser = await userDataSource.create(
    ...
    await userDataSource.delete(updatedUser.id);
    print("Deleted user");
  }

  Future<void> tryTodoApi() async {
    final todos = await todoDataSource.getAll();
    print("All todos: $todos");
    final firstTodo = await todoDataSource.getById(todos.first.id);
    print("First todo: $firstTodo");
    final createdTodo = await todoDataSource.create(
    ...
    await todoDataSource.delete(updatedTodo.id);
    print("Deleted todo");
  }
